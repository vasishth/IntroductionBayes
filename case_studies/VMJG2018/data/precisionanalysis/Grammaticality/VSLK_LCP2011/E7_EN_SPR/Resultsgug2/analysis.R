...
## question responses
alldataq <- read.table("allquestions.txt")
...
alldataq.gug$gram <- ifelse(alldataq.gug$condition%in%c("c"),1,-1)
...
means.subj.gug <- (with(alldataq.gug,tapply(correct,IND=list(subject,condition),mean,na.rm=TRUE))*100)
...
## post-V1
(fm.postv1 <- lmer(log(rt)~condition+(1|subj)+(1|item),subset(gram,(condition=="c" & pos==9) | (condition=="d" & pos==8)  & rt<2500)))

(fm.postv1 <- lmer(log(rt)~condition+(1|subj)+(1|item),subset(gram,(condition=="c" & pos==9) | (condition=="d" & pos==8))))
