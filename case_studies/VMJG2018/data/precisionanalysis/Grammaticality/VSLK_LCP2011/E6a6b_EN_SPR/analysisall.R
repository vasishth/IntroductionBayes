...
## question responses
alldata <- read.table("data/e6_all.txt")

#subset(alldata,subject==101 & run=="first" & comma=="nocomma")
#subset(alldata,subject==101 & run=="first" & comma=="comma")

colnames(alldata) <- c("run","comma","subj","expt","item","condition","question","response","correct","rt")

alldata$subject <- factor(paste(alldata$subj,alldata$run,alldata$comma,sep=""))
...
commadataq <- subset(alldataq,comma=="comma")
unique(commadataq$subject)

summary(commadataq)

with(commadataq,tapply(correct,subject,mean))

with(subset(commadataq,expt=="gug"),tapply(correct,condition,mean))

lmer(correct~condition+(1|subject)+(1|item),subset(commadataq,expt=="gug"),family=binomial())

summary(alldataq)

## mean and se of all responses:
mean(with(subset(alldataq,comma=="comma"),tapply(correct,subject,mean)))
se(with(subset(alldataq,comma=="comma"),tapply(correct,subject,mean)))
...
colnames(data) <- c("run","comma","subj","expt","item","condition","pos","word","dummy2","rt","subject")
...
c <- ifelse(gram$comma=="comma",-1,1)

gram$g <- g
gram$c <- c

length(unique(gram$subject)) ## 66
length(unique(subset(gram,comma=="comma")$subject)) ## 41
length(unique(subset(gram,comma=="nocomma")$subject)) ## 25


## V3
## comma only
(fm.v3 <- lmer(log(rt)~g+(1|subject)+(1|item),
               subset(gram,
                      comma=="comma" & pos==6 & rt<2000)))
...
## no comma:
(fm.v3 <- lmer(log(rt)~g+(1|subject)+(1|item),
               subset(gram,comma=="nocomma" & pos==6 & rt<2000)))

(fm.v3 <- lmer(log(rt)~g+(1|subject)+(1|item),
               subset(gram,comma=="nocomma" & pos==6)))

## both together:
(fm.v3 <- lmer(log(rt)~g*c+(1|subj)+(1|item),subset(gram, pos==6 & rt<2000)))

(fm.v3 <- lmer(log(rt)~g*c+(1|subj)+(1|item),subset(gram, pos==6)))

qq.plot(resid(fm.v3))

r <- ifelse(gram$run=="first",-1,1)

gram$r <- r

## V1
(fm.v1 <- lmer(log(rt)~g+(1|subject)+(1|item),
               subset(gram,
                      comma=="comma" &
                      (condition%in%c("a","c") & pos==8) |
                      (condition%in%c("b","d") & pos==7))))

(fm.v1 <- lmer(log(rt)~g+(1|subject)+(1|item),
               subset(gram,
                      comma=="comma" &
                      (condition%in%c("a","c") & pos==8) |
                      (condition%in%c("b","d") & pos==7) & rt<2000)))
...
(fm.v1 <- lmer(log(rt)~g*c+(1|subject)+(1|item),
               subset(gram,
                      (condition%in%c("a","c") & pos==8) |
                      (condition%in%c("b","d") & pos==7))))
...
## post-V1
(fm.postv1 <- lmer(log(rt)~g*r+(1|subject)+(1|item),
                   subset(gram,
                          comma=="comma" &
                          (condition%in%c("a","c") & pos==9) |
                          (condition%in%c("b","d") & pos==8)  & rt<2000)))

(fm.postv1 <- lmer(rt~g*r+(1|subject)+(1|item),
                   subset(gram,
                          comma=="comma" &
                          (condition%in%c("a","c") & pos==9) |
                          (condition%in%c("b","d") & pos==8)  & rt<2000)))
...
(fm.postv1 <- lmer(log(rt)~g+(1|subject)+(1|item),
                   subset(gram,
                          comma=="comma" &
                          (condition%in%c("a","c") & pos==9) |
                          (condition%in%c("b","d") & pos==8))))
...
createPS("expt6plot.ps")

matplot(ab.means[1,],
        ylim=c(min(ci.lower.c,na.rm=T)-50,max(ci.upper.c,na.rm=T)+50),
        type="b",pch=16,xaxt="n",
        main="Experiment 6",cex.main=2,lwd=3,ylab="",
        cex.axis=1.8)

lines(ab.means[2,],lwd=3,col=gray(0.5),lty=2)
points(ab.means[2,],pch=19,col=gray(0.5))
