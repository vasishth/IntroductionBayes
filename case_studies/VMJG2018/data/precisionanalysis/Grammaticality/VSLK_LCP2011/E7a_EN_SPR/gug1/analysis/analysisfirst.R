...
#gram <- subset(data.gug,pos%in%c(6,7,8,9) & run=="first")
gram <- subset(data.gug,pos%in%c(6,7,8,9) & run=="second")
...
## post-V1
(fm.postv1 <- lmer(log(rt)~condition+(1|subj)+(1|item),subset(gram,(condition=="a" & pos==9) | (condition=="b" & pos==8)  & rt<2000)))
