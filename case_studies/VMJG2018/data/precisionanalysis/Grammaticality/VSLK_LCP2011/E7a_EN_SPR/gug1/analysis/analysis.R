...
## question responses
alldataq <- read.table("allqgug1.txt")
colnames(alldataq) <- c("subject","subj","expt","item","condition","question","response","correct","qrt")
...
## gram: pos 6,7,8,9
## ungram 6 7 8
data <- read.table("datagug1.txt")

colnames(data) <- c("subj","subject","expt","item","condition","pos","word","dummy2","rt")
...
(fm.v1 <- lmer(log(rt)~condition+(1|subj)+(1|item),subset(gram,(condition=="a" & pos==8) | (condition=="b" & pos==7 & rt<1500))))

qq.plot(resid(fm.v1))

## post-V1
(fm.postv1 <- lmer(log(rt)~condition+(1|subj)+(1|item),subset(gram,(condition=="a" & pos==9) | (condition=="b" & pos==8)  & rt<1500)))
...
matplot(ab.means[1,],
        ylim=c(min(ci.lower.b,na.rm=T),max(ci.upper.a,na.rm=T)),
