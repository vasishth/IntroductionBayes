---
title: "Data analysis of Wu, Kaiser, Vasishth Cognitive Science, 2016"
...
Note: 
Revision following second round of reviews. The data were collected by Fuyun Wu and the initial analyses were done by her. I redid everything just to be sure and to reproduce her results.
...
condaroi<-c("RCNP","RCV","de","headnoun","Adv","MainV","MainO")
condbroi<-c("CL","RCNP","RCV","de","headnoun","Adv","MainV","MainO")
condcroi<-c("BEI","RCNP","RCV","de","headnoun","Adv","MainV","MainO")
conddroi<-c("CL","BEI","RCNP","RCV","de","headnoun","Adv","MainV","MainO")
...
e1$roi<-factor(e1$roi,levels=c("CL","BEI","RCNP","RCV","de","headnoun","Adv","MainV","MainO"))

e1$ID<-factor(paste("e1_",e1$ID,sep=""))
colnames(e1)[3]<-"subject"
e1$expt<-"e1"
e1<-e1[,c(3,5,1,10,11,12)]
write.table(e1,file="../data/e1clean.txt")
...
write.table(e1merged,file="../data/e1clean.txt")
...
## 7 main verb MainV
## 8 main obj MainO
e2$roi<-ifelse(e2$relabelpos==2,"RCNP",ifelse(e2$relabelpos==3,"RCV",
...
Comparing the distribution of reading times at de for the three transformed reading times: 
...
rois<-rep(c("RCNP","RCV","de","headnoun","Adv","MainV","MainO"),each=3)
rois<-factor(rois,levels=c("RCNP","RCV","de","headnoun","Adv","MainV","MainO"))
...
Statistical significant at threshold 2.5:
...
```{r fig.width=11,fig.height=10}
plotresultse1<-plotresults(resultse1,maintitle="Experiment 1",
                          ylabel="Raw RTs")
print(plotresultse1)
```
...
```{r}
e1<-read.table("../data/e1clean.txt",header=TRUE)
e1crit<-subset(e1,roi%in%c("RCNP","RCV","de","headnoun","Adv","MainV","MainO"))
...
e1crit$roi<-factor(e1crit$roi,levels=c("RCNP","RCV","de","headnoun","Adv","MainV","MainO"))
...
regionplote1<-regionplot(M.id.w,maintitle="Experiment 1",legendpos=c(0.1,.5))
```


```{r fig.width=8,fig.height=8}
multiplot(figresultse1rrt,
          regionplote1,cols=1)
```
...
```{r fig.width=11,fig.height=10}
plotresultse2rrt<-plotresults(resultse2rrt,maintitle="Experiment 2")
print(plotresultse2rrt)
```
...
```{r fig.width=11,fig.height=10}
plotresultse2<-plotresults(resultse2,maintitle="Experiment 2",ylabel="Raw RTs")
```


```{r}
e2crit<-subset(e2,
               roi%in%c("RCNP","RCV","de","headnoun","Adv","MainV","MainO"))

e2crit$roi<-as.factor(as.character(e2crit$roi))
unique(e2crit$roi)

e2crit$roi<-factor(e2crit$roi,
                   levels=c("RCNP","RCV","de","headnoun","Adv","MainV","MainO"))
...
regionplote2<-regionplot(M.id.w,maintitle="Experiment 2",legendpos=c(0.05,0.4),ymin=400,ymax=1050)
```
...
multiplot(plotresultse2rrt,
          regionplote2,
          cols=1)
```

```{r plotresultse1e2,fig.width=10,fig.height=12}
